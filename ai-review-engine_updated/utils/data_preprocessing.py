...
class DataPreprocessor:
    ...
    def preprocess_review(self, review: str) -> Dict:
        """
        Complete preprocessing pipeline for a single review with error handling
        ...
        Raises:
            ValueError: If review is invalid or empty
            RuntimeError: If preprocessing steps fail
        """
        ...
        try:
            ...
            if not cleaned:
                raise ValueError("Text cleaning resulted in empty string")
            ...
            # Validate final processed text
            if len(processed_text.split()) < 3:
                logger.warning(f"Very short processed text: {processed_text}")
            ...
            logger.error(f"Failed review text: {review[:100]}...")
