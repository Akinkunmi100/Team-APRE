...
def get_primary_dataset():
    """
    Get the primary cleaned dataset with validation.
    ...
    try:
        if _cached_dataset is None:
            ...
            if not isinstance(_cached_dataset, pd.DataFrame):
                raise ValueError(f"Expected DataFrame, got {type(_cached_dataset)}")
            
            if len(_cached_dataset) == 0:
                raise ValueError("Dataset is empty")
            
            # Log dataset statistics
            logger.info(f"Loaded dataset with {len(_cached_dataset)} records")
            logger.info(f"Columns: {list(_cached_dataset.columns)}")
            
            if 'rating' in _cached_dataset.columns:
                avg_rating = _cached_dataset['rating'].mean()
                logger.info(f"Average rating: {avg_rating:.2f}")
        
        return _cached_dataset
        
    except Exception as e:
        logger.error(f"Error accessing primary dataset: {str(e)}")
        raise
...
def generate_fake_realtime_data():
    """
    Generate fake real-time data for dashboard demos.
    Uses actual reviews but simulates real-time arrival.
    
    Returns:
        dict: Simulated real-time event
    """
    dataset = get_primary_dataset()
    ...
    # Create event
    event = {
        'type': np.random.choice(['new_review', 'rating_update', 'sentiment_shift']),
        'timestamp': pd.Timestamp.now().isoformat(),
        'product': row['product'],
        'brand': row['brand'],
        'review_text': row['review_text'][:200] if len(row['review_text']) > 200 else row['review_text'],
        'rating': row.get('rating', np.random.choice([3, 4, 5])),
        'sentiment': np.random.choice(['positive', 'neutral', 'negative'], p=[0.6, 0.25, 0.15]),
        'user_id': row.get('user_id', f"USER_{np.random.randint(1000, 9999)}")
    }
