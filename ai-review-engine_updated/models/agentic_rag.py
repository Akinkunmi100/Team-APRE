...
class OrchestratorAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "orchestrator_001"):
        ...
        self.agents = {}
        self.execution_plan = []
        self.data_availability_cache = {}  # Cache for data availability checks
    ...
    async def create_execution_plan(self, task: AgentTask) -> List[Dict]:
        """Create an execution plan for a complex task with intelligent web search integration"""
        plan = []
        
        # Extract phone model from task context or description
        phone_model = task.context.get('phone_model', self._extract_phone_model(task.description))
        
        # Check data availability first
        data_availability = await self._check_data_availability(phone_model)
        ...
        if task_type == "comparison":
            plan = [
                {'agent': 'researcher', 'action': 'gather_info', 'priority': 1, 'context': {'phone_model': phone_model}},
            ...
            # Add web search if local data is insufficient
            if data_availability.get('recommendation') in ['full_web_search', 'partial_web_search']:
                plan.insert(0, {
                    'agent': 'web_search', 
                    'action': 'search_comprehensive', 
                    'priority': 0,
                    'context': {'phone_model': phone_model, 'search_type': 'comprehensive'}
                })
            ...
            # Add web search for missing phone data
            if data_availability.get('recommendation') != 'use_cached_data':
                plan.insert(0, {
                    'agent': 'web_search', 
                    'action': 'search_specifications_reviews', 
                    'priority': 0,
                    'context': {'phone_model': phone_model, 'search_type': 'specifications'}
                })
                
        elif task_type == "analysis":
            plan = [
                {'agent': 'analyst', 'action': 'deep_analysis', 'priority': 2},
                {'agent': 'summarizer', 'action': 'create_report', 'priority': 3}
            ]
            
            # Prioritize web search for analysis tasks to get fresh data
            if data_availability.get('recommendation') in ['full_web_search', 'partial_web_search']:
                plan.insert(0, {
                    'agent': 'web_search', 
                    'action': 'search_reviews_sentiment', 
                    'priority': 0,
                    'context': {'phone_model': phone_model, 'search_type': 'reviews'}
                })
            
            # Add data validation step
            plan.insert(-1, {'agent': 'validator', 'action': 'validate_data', 'priority': 1.5})
                
        else:
            # Default plan with intelligent web search
            plan = [
            ...
            if data_availability.get('recommendation') != 'use_cached_data':
                plan.insert(0, {
                    'agent': 'web_search', 
                    'action': 'search_phone_data', 
                    'priority': 0,
                    'context': {'phone_model': phone_model, 'search_type': 'comprehensive'}
                })
        
        # Store metadata about data availability decision
        for step in plan:
            step['data_availability'] = data_availability
            if phone_model:
                step['context'] = step.get('context', {})
                step['context']['phone_model'] = phone_model
        
        self.execution_plan = plan
        logger.info(f"Created execution plan with {len(plan)} steps, web search: {'web_search' in [s['agent'] for s in plan]}")
        return plan
    ...
    async def aggregate_results(self, results: List[Any]) -> Dict[str, Any]:
        ...
        for result in results:
            if isinstance(result, dict):
                ...
                if result.get('agent') == 'web_search':
                    aggregated['web_search_used'] = True
                    if 'sources_searched' in result.get('result', {}).get('search_metadata', {}):
                        aggregated['data_sources'].extend(
                            result['result']['search_metadata']['sources_searched']
                        )
        ...
        if aggregated['details']:
            ...
            if aggregated['web_search_used']:
                aggregated['summary'] += " (including fresh web data)"
        
        return aggregated
    
    async def _check_data_availability(self, phone_model: str) -> Dict[str, Any]:
        """Check data availability for a phone model"""
        if not phone_model:
            return {'recommendation': 'full_web_search', 'reason': 'no_phone_model'}
        
        # Check cache first
        if phone_model in self.data_availability_cache:
            cached_result = self.data_availability_cache[phone_model]
        ...
        # Check with web search agent if available
        if 'web_search' in self.agents:
            web_agent = self.agents['web_search']
            availability = await web_agent.check_data_availability(phone_model)
            
            # Cache the result
            self.data_availability_cache[phone_model] = {
            ...
            # Default to web search if no web search agent available
            return {
                'phone_model': phone_model,
                'recommendation': 'full_web_search',
                'reason': 'no_web_search_agent_available'
            }
    
    def _extract_phone_model(self, description: str) -> Optional[str]:
        """Extract phone model from task description"""
        ...
        # Try to extract any phone-like model
        words = description.split()
        for i, word in enumerate(words):
            if any(brand.lower() in word.lower() for brand in ['iphone', 'galaxy', 'pixel', 'oneplus']):
                # Try to get next 1-2 words as model
                model_parts = [word]
                for j in range(1, min(3, len(words) - i)):
                    if words[i + j].replace('.', '').replace('-', '').isalnum():
                        model_parts.append(words[i + j])
                    else:
                        break
...
class AgenticRAGSystem:
    ...
    def __init__(self, enable_web_search: bool = True):
        ...
        if enable_web_search:
            try:
                from models.web_search_agent import WebSearchAgent
